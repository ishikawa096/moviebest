...
RSpec.describe 'Api::V1::Lists', type: :request do
  ...
  describe '#edit' do
    it 'レコードを更新できる' do
      create(:list, :skip_validation, :with_theme, theme_capacity: 2,
                    id: update_params[:id], comment: 'comment', user:,
                    movies: [create(:movie, :skip_validation, id: 1, list:),
                             create(:movie, :skip_validation, id: 2, list:)])
    ...
    it 'バリデーションエラーがあるときエラーコードとエラー内容を返す' do
      create(:list, :skip_validation, :with_theme, theme_capacity: 2,
                    id: invalid_params[:id], comment: 'comment', user:,
                    movies: [create(:movie, :skip_validation, id: 1, list:),
                             create(:movie, :skip_validation, id: 2, list:)])
    ...
    it 'listの作者でないユーザーは更新できない' do
      ...
      create(:list, :skip_validation, :with_theme, theme_capacity: 2,
                    id: update_params[:id], comment: 'comment', user: another_user,
                    movies: [create(:movie, :skip_validation, id: 1, list:),
                             create(:movie, :skip_validation, id: 2, list:)])
      patch api_v1_list_path(update_params[:id]), headers: api_and_user_auth, params: update_params
      expect(response).to have_http_status(:unauthorized)
