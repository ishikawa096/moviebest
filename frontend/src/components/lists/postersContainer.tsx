...
const PostersContainer = ({ children, movies }: { children: ReactNode; movies: Array<Movie> }) => {
  ...
  if (length < 6) {
    return <div className='mb-10 flex flex-row flex-no-wrap justify-center w-screen bg-white'>{children}</div>
  } else {
    return (
      <div className='mb-10 flex justify-center w-screen bg-white'>
        <div
          className={`${
