...
const ListCard = ({ theme, user, movies }: Props) => {
  ...
  const listColumn = movies.map((movie) => (
    <div key={movie.id} className='flex flex-row backdrop-blur-3xl backdrop-brightness-3xl italic text-black rounded-sm overflow-hidden backdrop-saturate-[3]'>
      <div className={`text-center mr-0.5 flex font-logo w-5 bg-opacity-30 items-center ${movie.position + 1 >= 10 ? 'pl-0 text-sm' : 'text-lg' }`}>{movie.position + 1}</div>
      <div className={`'pl-1 pr-3 py-1 flex items-center font-medium tracking-tighter w-full bg-opacity-50 bg-white whitespace-nowrap overflow-x-scroll ${movie.title.length > 12 ? 'text-base' : 'text-xl' }`}>{movie.title}</div>
    </div>
  ))

  return (
    <div className='relative flex flex-col w-80 sm:w-[22rem] h-[fit-content] min-h-[30rem] mr-auto ml-auto justify-center items-center rounded-2xl overflow-hidden'>
      <Image
      ...
      <div className='absolute flex flex-col w-full h-full z-10 rounded-2xl'>
        <Link href={`/themes/${theme.id}`}>
          <a>
            <div className='p-4 text-center text-xl text-white backdrop-brightness-50 backdrop-blur-lg backdrop-saturate-[5] hover:underline'>
              <h1># {theme.title}</h1>
            </div>
          </a>
        </Link>
        <Link href={`/lists/${movies[0].listId}`}>
          <a className='contents'>
            <div className='flex flex-col justify-around h-full px-3 mb-10'>{listColumn}</div>
          </a>
        </Link>
